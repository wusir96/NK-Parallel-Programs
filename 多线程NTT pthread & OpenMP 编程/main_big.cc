...
void fRead(long long *a, long long *b, int *n, long long*p, int input_id){//long 
    // 数据输入函数
...
void fCheck(long long *ab, int n, int input_id){
    ...
    for (int i = 0; i < n * 2 - 1; i++){
        long long expected_val; // MODIFIED: int to long long
        if (!(fin >> expected_val)) {
...
long long mod_pow(long long a, long long b, int p) {
    long long res = 1;
    a %= p; // If a is negative, a % p might be negative.
            // (a % p + p) % p would be safer if a could be negative.
            // However, 'a' here is usually a base like 3 or a coefficient.
    while (b > 0) {
        if (b & 1) res = res * a % p;
        a = a * a % p;
        b >>= 1;
    }
    return res; // res will be in [0, p-1] if p > 0.
}
...
const int MODULI[MOD_COUNT] = {469762049, 998244353, 167772161,1004535809};  // 三个互质模数
...
CrtConstants() {
    ...
    std::cout << "DEBUG CrtConstants: M = " << M << std::endl;
    for (int i = 0; i < MOD_COUNT; i++) {
        M_divs[i] = M / MODULI[i];
        ...
        std::cout << "DEBUG CrtConstants: MODULI[" << i << "]=" << MODULI[i]
                  << ", M_divs[" << i << "]=" << M_divs[i]
                  << ", M_div_mod_mi[" << i << "]=" << M_div_mod_mi
                  << ", M_invs[" << i << "]=" << M_invs[i] << std::endl;
        // Sanity check for M_invs: (M_div_mod_mi * M_invs[i]) % MODULI[i] should be 1
        if (M_div_mod_mi != 0) { // Avoid division by zero if M_div_mod_mi is 0 (should not happen)
             cpp_int check_inv = (M_div_mod_mi * M_invs[i]) % cpp_int(MODULI[i]);
             if (check_inv < 0) check_inv += MODULI[i];
             if (check_inv != 1) {
                 std::cout << "ERROR CrtConstants: Inverse check failed for MODULI[" << i << "]! Got " << check_inv << std::endl;
             }
        }
    }
}
...
void* pthread_crt_ntt_worker(void *arg) {
    ...
    long long *a_input = args_ptr->a; // Renamed a to a_input
    long long *b_input = args_ptr->b; // Renamed b to b_input
    int n_val = args_ptr->n;    // Renamed n to n_val
    int p_mod = args_ptr->p;    // Renamed p to p_mod
    long long  *c_output = args_ptr->c; // Renamed c to c_output
    ...
    ntt(ta, len, -1, p_mod);
    // MODIFIED: Copy int results from ta to long long c_output element by element
    for (int i = 0; i < (2 * n_val - 1); ++i) {
        c_output[i] = static_cast<long long>(ta[i]);
    }
    // memcpy(c_output, ta, (2 * n_val - 1) * sizeof(int)); // This was incorrect
    delete[] ta;
    delete[] tb;
    return NULL;
}
...
void crt_combine(long long*results[], int n_orig, long long p_target, long long *c_final) {
    ...
    for (int i = 0; i < result_len; i++) {
        ...
        c_final[i] = static_cast<long long>(final_value_mod_p); // MODIFIED HERE
    }
}
// pthread CRT NTT多项式乘法
// 优化的pthread CRT NTT多项式乘法  //p long long
void pthread_crt_ntt_multiply(long long *a, long long*b, long long*c, int n, long long p_target) { // Renamed p to p_target
    pthread_t threads[MOD_COUNT];
    CrtThreadArgs args[MOD_COUNT];
     long long  *results[MOD_COUNT]; // Array of pointers to results for each modulus
    ...
    for (int i = 0; i < MOD_COUNT; i++) {
        ...
        results[i] = new long long[2 * n - 1]; // Only need 2*n-1 for final result storage
...
// OpenMP CRT NTT多项式乘法 long long 
void openmp_crt_ntt_multiply(long long *a, long long *b, long long *c, int n, long long p_target) { 
    long long *results[MOD_COUNT];
    int output_len = 2 * n - 1; 

    for (int i = 0; i < MOD_COUNT; i++) {
        results[i] = new  long long[output_len]; 
    }
    ...
    for (int i = 0; i < MOD_COUNT; i++) {
        ...
        // MODIFIED: Copy int results from ta to long long results[i] element by element
        for (int k = 0; k < output_len; ++k) {
            results[i][k] = static_cast<long long>(ta[k]);
        }
        // memcpy(results[i], ta, output_len * sizeof(int)); // This was incorrect
...
// 测试各种实现的性能
// 测试各种实现的性能 //修改p
void test_performance(long long  *a, long long  *b, long long  *c, int n, long long p, int current_input_id) { // 添加 current_input_id 参数
...
void big_mod_ntt_multiply(long long *a, long long *b, long long*c, int n, cpp_int p_target_big) { 
    int *results[MOD_COUNT]; // These are intermediate results from NTTs with small moduli
    int output_len = 2 * n - 1;
    ...
    for (int i = 0; i < MOD_COUNT; i++) {
        ...
        memcpy(results[i], ta, output_len * sizeof(int)); // results[i] is int*, ta is int*, this is fine
    ...
    for (int i = 0; i < output_len; i++) {
        cpp_int current_sum_mod_M = 0;
        for (int j = 0; j < MOD_COUNT; j++) {
            cpp_int val_j = results[j][i]; // results[j][i] is int
            val_j = (val_j % MODULI[j] + MODULI[j]) % MODULI[j];
        ...
        c[i] = static_cast<long long>(final_val); // MODIFIED HERE (c is long long*)
    }
...
for(int i = test_begin; i <= test_end; ++i) {
        ...
        long long p_;
        fRead(a, b, &n_, &p_, i);
        std::cout << "DEBUG: Input a (first 5): ";
for(int k=0; k<std::min(n_, 5); ++k) std::cout << a[k] << " ";
std::cout << std::endl;
std::cout << "DEBUG: Input b (first 5): ";
for(int k=0; k<std::min(n_, 5); ++k) std::cout << b[k] << " ";
std::cout << std::endl;
        
        std::cout << "测试用例 " << i << "：n = " << n_ << ", p = " << p_ << std::endl;
        
        memset(ab, 0, (2*n_ -1) * sizeof(long long)); // MODIFIED: sizeof(int) to sizeof(long long)
        test_performance(a, b, ab, n_, p_, i); 
       
        if (p_ > INT_MAX || p_ == 1337006139375617LL) {
            std::cout << "\n大模数专用算法测试:" << std::endl;
            memset(ab, 0, (2*n_ -1) * sizeof(long long)); // MODIFIED: sizeof(ab) to specific length for consistency
                                                       // sizeof(ab) was correct for zeroing full static array,
                                                       // but (2*n_ -1) * sizeof(long long) is more precise for the used part.
            
            auto start = std::chrono::high_resolution_clock::now();
            cpp_int big_p = p_;
            // This specific check for 1337...LL to create cpp_int from string is a bit redundant
            // if p_ is already the correct long long. cpp_int big_p = p_; is usually sufficient.
            // However, if p_ could be a different large number that fits long long but might have issues
            // with direct cpp_int conversion (unlikely for positive values), string is safer.
            // For now, let's keep it as it might be intentional for very specific large values.
            if (p_ == 1337006139375617LL) { // This if can be simplified if p_ is always the source
                 big_p = cpp_int("1337006139375617");
            } // else big_p is already p_ (long long)
